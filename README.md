...
本仓库根目录同时是一个 Codex 插件，插件相关文件会随 Git 一起发布：

```text
.codex-plugin/plugin.json              插件名称、说明和界面信息
.mcp.json                              插件自带的 SellerSprite MCP 配置
skills/sellersprite-research/SKILL.md  SellerSprite 与亚马逊研究触发规则
scripts/start-mcp.mjs                  跨平台 Node 启动器
```

安装插件后，在 Codex 插件列表中应显示 **SellerSprite MCP**。首次安装或更新后刷新插件页面，并创建一个新任务，让 Codex 载入新插件的 Skill 和 MCP 工具。可以用下面的提示词验证触发：

```text
使用 SellerSprite MCP 检查登录状态，然后分析 ASIN B00CKFL93K 的流量词。
```

插件触发后会优先调用 `sellersprite_session_status`，登录态缺失时调用 `sellersprite_login`，而不是用普通网页搜索替代卖家精灵数据。
...
推荐安装为 Codex 插件后，让 Agent 调用 `sellersprite_login`。工具会在 Windows、macOS 和 Linux 上寻找 Chrome、Edge、Brave 或 Chromium，并使用当前系统对应的持久化配置目录。
